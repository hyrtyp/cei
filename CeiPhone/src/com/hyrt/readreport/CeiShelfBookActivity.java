...
public class CeiShelfBookActivity extends Activity implements OnClickListener {
			...
			case 10:
				...
				if (isDownload) {
					 					...
					 					new Handler().postDelayed(new Runnable() {
					 						...
					 						@Override
					 						public void run() {
					 							//selfAdapter.viewBookSelf.performClick();
					 							ShelfDownload(selfAdapter.viewBookSelf,0);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		isDownloadintent = getIntent();
		isDownload = isDownloadintent.getBooleanExtra("isDownload", false);
		initView();
		initData();
	}
	...
	private void ShelfDownload(View arg1, final int arg2) {
		 		...
		 		if (!report.getIsLoad().equals("yes")) {
		 			...
		 			try {
		 				if (loadThreads.get(report.getId()) != null) {
		 					MyTools.exitShow(CeiShelfBookActivity.this,
		 							CeiShelfBookActivity.this.getWindow()
		 									.getDecorView(), "正在下载！");
		 					return;
		 				} else {
